...
@[substSimp] theorem substAt_impl : ₛ[α →ₑ β, k ↦ v] = (ₛ[α, k ↦ v] →ₑ ₛ[β, k ↦ v]) := by sorry
@[substSimp] theorem substAt_quorum : ₛ[⊡ₑ α, k ↦ v] = (⊡ₑ ₛ[α, k ↦ v]) := by sorry
@[substSimp] theorem substAt_bound : Term.substAt 0 v (.bound 0) = (.val v : Term V 0) := by sorry
@[substSimp] theorem substAt_predicate : ₛ[Expr.predicate p (.bound 0), 0 ↦ v] = (Expr.predicate p (.val v) : Expr V P 1) := by sorry
